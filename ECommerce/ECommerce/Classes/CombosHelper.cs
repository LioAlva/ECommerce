...
{
    public class CombosHelper:IDisposable//es oara cerrar conexiones
    {
        ...
        {
            var categories = db.Categories.Where(u=>u.CompanyId==companyId).ToList();
        ...
        {
            var taxes = db.Taxes.Where(u => u.CompanyId == companyId).ToList();
            taxes.Add(new Tax
            {
                TaxId = 0,
                Description = "[Select a Company ... ]",
